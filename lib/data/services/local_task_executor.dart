...
class LocalTaskExecutor {
  ...
  Future<void> _executeTask(Task task) async {
    ...
    var acquiredConcurrencyKey = false;
    var markedExecutionStarted = false;
    try {
      concurrencyKey = _concurrencyKeyForTask(task);
      if (concurrencyKey != null) {
        acquiredConcurrencyKey = _activeConcurrencyKeys.add(concurrencyKey);
      ...
      if (markedExecutionStarted) {
        await _clearTaskExecutionMarker(task.id);
      }
      if (acquiredConcurrencyKey && concurrencyKey != null) {
